...
plot_grid_function <- function(a = 6,  p = .2, sigma = 1/5, n0 = 7,
                               k = 15, k_optimal = FALSE , sigma_interval = 0.5,
                               sigma_end = 40){
  likelihood_values <- loglikelihood_values(a, p, sigma , n0, k, k_optimal, sigma_interval, sigma_end)
  likelihood_values_melt <- melt(likelihood_values$grid_values, value.name = "log-likelihood", varnames = c("sigma", "p"))
  if(k_optimal == FALSE){
    if(k < n0){
      ggplot(likelihood_values_melt, aes(x = p, y = sigma, fill = `log-likelihood`)) +
        ggtitle( paste("space", as.character(k), sep = " ")) +
        theme_bw() + geom_tile(color = "white", lwd = .1) +
